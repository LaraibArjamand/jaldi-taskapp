...
def create_task_id(headers, task_payload, user_session_with_tasks):
    create_response = user_session_with_tasks.post(config.TASK_ENDPOINT, headers=headers, data=task_payload)
...
def test_task_create(headers, task_payload, user_session_with_tasks):
    ...
    list_response = user_session_with_tasks.get(config.TASK_ENDPOINT, headers=headers)
...
def test_task_update(headers, user_session_with_tasks, create_task_id):
    ...
    update_response = user_session_with_tasks.patch(f"{config.TASK_ENDPOINT}/{create_task_id}", headers=headers, json=update_data)
...
def test_task_delete(headers, user_session_with_tasks, create_task_id):
    user_session_with_tasks.delete(f"{config.TASK_ENDPOINT}/{create_task_id}")
